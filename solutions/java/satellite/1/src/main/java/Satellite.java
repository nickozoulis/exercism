...
public class Satellite {
    ...
    private Node reconstructTree(ArrayList<Character> preorderInput, List<Character> inorderInput) {
        ...
        parent.left = reconstructTreeRecursive(preorderInput, inorderInput.subList(0, indexOfParent));

        preorderInput.remove(0);
        parent.right = reconstructTreeRecursive(preorderInput, inorderInput.subList(indexOfParent + 1, inorderInput.size()));

        return parent;
    }

    private Node reconstructTreeRecursive(ArrayList<Character> preorderInput, List<Character> inorderInput) {
        final var parent = new Node(preorderInput.get(0));
